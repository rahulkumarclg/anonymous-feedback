...
impl Anonymousfeedback {
    pub fn send_feedback(env: Env, feedback_msg: String) -> u32 {
        let mut fb_count: u32 = env.storage().instance().get(&COUNT_FB).unwrap_or(0);
            fb_count += 1;
    ...
    pub fn fetch_feedback(env: Env, fb_id: u32) -> Feedback{
